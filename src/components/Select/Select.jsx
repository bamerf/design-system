...
import { DownArrow } from '../../assets/svg/icons.jsx';
...
export default function Select({
  ...
  const clickOutsideHandler = (event) => {
    if (
      dropdownListRef.current.contains(
        event.target || activatorRef.current.contains(event.target)
      )
    )
      return;
    setIsopen(false);
            ...
            ? items.map((item, index) => {
                return (
                  <ListItem
                    ...
                    onKeyUp={(event) => handleKey(event, item)}
                  >
                    {item}
                  </ListItem>
                );
